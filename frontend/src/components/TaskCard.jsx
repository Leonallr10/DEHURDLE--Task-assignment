...
export default function TaskCard({ task, onUpdate }) {
  ...
  return (
    <article
      className={`group relative rounded-2xl border bg-white p-5 shadow-sm transition-all duration-200 hover:-translate-y-0.5 hover:shadow-md ${
        loading ? 'pointer-events-none opacity-60' : ''
      } ${task.status === 'done' ? 'border-emerald-100' : 'border-slate-200'}`}
    >
      <div className="flex items-start justify-between gap-4">
        <div className="min-w-0 flex-1">
          ...
          <h3 className={`text-base font-semibold text-slate-900 ${task.status === 'done' ? 'line-through opacity-70' : ''}`}>
            {task.title}
          </h3>
          {task.description && (
            <p className="mt-1.5 text-sm leading-relaxed text-slate-600">{task.description}</p>
          )}
          {task.dueDate && (
            <p className={`mt-2 inline-flex items-center gap-1.5 text-xs font-medium ${isOverdue ? 'text-red-600' : 'text-slate-500'}`}>
              <span>📅</span>
      ...
      {confirmDelete && (
        <div className="absolute inset-0 flex items-center justify-center rounded-2xl bg-white/95 p-4 backdrop-blur-sm">
          <div className="text-center">
            <p className="text-sm font-medium text-slate-800">Delete this task?</p>
